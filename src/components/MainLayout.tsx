...
export default function MainLayout({
  ...
  React.useEffect(() => {
    // Keep isDark state in sync with DOM class changes (like when toggling on other screens)
  ...
  const desktopNavItems = [
    ...
    { id: "save_to_buy", icon: ShoppingCart, label: "Shop" },
  ...
  return (
    <div className="h-dvh bg-background flex flex-col md:flex-row overflow-hidden">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-surface border-r border-outline-variant/50 p-6 sticky top-0 h-screen">
        ...
        <div className="mt-auto flex flex-col gap-2">
          ...
          >
            ...
            <div
              className={`w-8 h-5 bg-primary/20 rounded-full p-0.5 flex items-center transition-all ${isDark ? "justify-end" : "justify-start"}`}
            >
      ...
      <div className="flex-1 flex flex-col max-w-full overflow-hidden">
            ...
            {activeScreen === "profile" ? (
              <>
                <button
                  ...
                  className="w-9 h-9 rounded-full overflow-hidden border border-outline-variant/30 relative z-10"
                >
                  <img
              ...
              <>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => onNavigate("profile")}
                    className="w-8 h-8 rounded-full overflow-hidden border border-outline-variant/30"
                  >
                    <img
                      src="https://i.pravatar.cc/150?u=a042581f4e29026704d"
                      alt="Profile"
          ...
          <header className="hidden md:flex items-center justify-between p-6 bg-background">
            <div>
              <h2 className="text-headline-md font-bold text-on-background capitalize">
                {activeScreen.replace(/_/g, " ")}
              </h2>
              <p className="text-on-surface-variant text-body-sm font-normal mt-1">
                Welcome back, John!
              </p>
            </div>
            ...
            <div className="flex items-center gap-4">
              <button
                onClick={() => onNavigate("notifications")}
                className="relative text-on-surface-variant hover:text-on-surface transition-colors p-2"
              >
                <Bell size={24} />
        ...
        <main
          className={`flex-1 overflow-y-auto relative scroll-smooth scrollbar-hide ${!["dashboard", "create_savings"].includes(activeScreen) ? "p-5 pb-24 md:pb-6" : ""}`}
        >
          {children}
        </main>
      </div>
      ...
      {/* Mobile Bottom Nav */}
      <nav
        className={`md:hidden fixed bottom-0 left-0 right-0 bg-surface border-t border-outline-variant/20 pt-2 pb-6 px-2 flex justify-around items-center z-30 transition-transform ${mobileNavItems.some((i) => i.id === activeScreen) ? "translate-y-0" : "translate-y-full opacity-0 pointer-events-none"}`}
      >
        {mobileNavItems.map((item) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => onNavigate(item.id)}
              className={`flex flex-col items-center gap-1.5 py-2.5 px-0 w-[64px] rounded-2xl transition-colors ${
                isActive
            ...
            >
              <Icon size={20} strokeWidth={isActive ? 2.5 : 2} />
              <span
                className={`text-[9px] tracking-wider uppercase text-center ${isActive ? "font-bold" : "font-medium"}`}
              >
                {item.label}
              </span>
            </button>
