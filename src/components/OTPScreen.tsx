    ...
    <div className="flex flex-col h-full bg-background">
      <header className="p-4 flex justify-start">
        <button
          ...
          className="w-10 h-10 flex items-center justify-center bg-surface hover:bg-surface-variant rounded-full text-on-background transition-colors border border-outline-variant/10"
        >
          <ArrowLeft size={20} />
        </button>
      </header>
      ...
      <div className="flex-1 flex flex-col justify-start items-center px-4 pt-16">
        <div className="w-full max-w-sm bg-surface rounded-[24px] p-8 flex flex-col items-center border border-outline-variant/10 shadow-sm relative">
          ...
          <h1 className="text-[32px] font-medium text-on-background tracking-tight mb-2">
            Security Check
          </h1>
          ...
          <div className="flex justify-center gap-2 mb-8">
            {otp.map((digit, idx) => {
              ...
              const isActive = idx === (firstEmptyIdx === -1 ? 5 : firstEmptyIdx);
              return (
                <input
                  key={idx}
                  // @ts-ignore
          ...
          <div className="flex flex-col items-center gap-3 mb-8 w-full">
            <div className="flex items-center gap-2 text-sm text-on-surface-variant font-medium">
              ...
              <span>
                Resend Code in{" "}
                <span className="font-semibold text-on-background">00:45</span>
              </span>
            </div>
